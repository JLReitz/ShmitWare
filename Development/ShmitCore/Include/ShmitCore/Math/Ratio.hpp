...
public:
    ...
    explicit constexpr operator intmax_t() const noexcept;
 ...
 */
template<class T>
constexpr static bool is_ratio_v = is_ratio<T>::value;
...
constexpr Ratio Ratio::Simplify(Ratio const& ratio) noexcept // Static method
{
    ...
    intmax_t divisor = gcd(simplified.numerator, simplified.denominator);
    bool     numerator_is_cleanly_divided {(simplified.numerator % divisor) == 0U};
    bool     denominator_is_cleanly_divided {(simplified.denominator % divisor) == 0U};
    if (numerator_is_cleanly_divided && denominator_is_cleanly_divided)
    {
